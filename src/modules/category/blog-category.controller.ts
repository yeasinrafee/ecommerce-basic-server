...
const createCategory = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const created = await blogCategoryService.createCategory({ name, image: imageUrl ?? null });
        ...
        if (newlyUploadedPublicId) {
            try {
                ...
                console.warn('Failed to cleanup uploaded blog category image after create failure', { newlyUploadedPublicId, err: (cleanupErr as Error).message });
            }
        }
...
const updateCategory = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const updated = await blogCategoryService.updateCategory(id, payload, newlyUploadedPublicId);
        ...
        if (newlyUploadedPublicId) {
            try {
                ...
                console.warn('Failed to cleanup newly uploaded blog category asset after update failure', { newlyUploadedPublicId, err: (deleteErr as Error).message });
            }
        }
...
const getCategories = async (req: Request, res: Response) => {
    ...
    const result = await blogCategoryService.getCategories({ page, limit, searchTerm });
...
const getCategory = async (req: Request, res: Response) => {
    const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
    const cat = await blogCategoryService.getCategoryById(id);
...
const getAllCategories = async (req: Request, res: Response) => {
    const cats = await blogCategoryService.getAllCategories();
